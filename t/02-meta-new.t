...
$blog = MT::Blog->load (1);
is($blog->meta('page_layout'), 'layout-wtt', "page_layout meta column is correct");
...
is($ref_blog->my_meta_hash->{something}, 'other', "New meta column has the right HASH value ({'something' => 'other'})");
...
is($ref_blog->my_meta_hash->{something}, 'else', "New meta column has the right HASH value ({'something' => 'else'}) after update");

MT::Object->driver->clear_cache; # reset ramcache to make sure data is coming from memcached
...
$ref_blog = MT::Blog->load(1);
ok(exists $ref_blog->my_meta_hash->{a_new}, "Meta column has a new key in its hash");

MT::Author->install_meta ({ column_defs => { 'field.my_field' => 'hash meta' }});
