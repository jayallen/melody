...
__PACKAGE__->install_properties( {
            column_defs =>
              { id => 'integer not null auto_increment', boo => 'asfdasf', },
...
sub _def {
    ...
    my $def = { not_null => $not_null, type => $type, %param, };
    return $def;
}
...
sub multikey_unique : Tests(1) {
    ...
    $orig->set_values(
                 { fkey => 7, type => 'awesome', value => 'these truths', } );
    $orig->save
...
sub short_index : Tests(4) {
  ...
  SKIP: {
        ...
        is_deeply(
                   ...
                   { columns => [qw( foo bar )], sizes => { bar => 10, }, },
