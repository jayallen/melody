...
MT::BackupRestore->restore_process_single_file( $h, \%objects, \%deferred,
         \@errors, MT->config->SchemaVersion, 0, sub { print $_[0], "\n"; } );
    ...
    foreach my $name ( keys %$oldies ) {
        ...
        for my $old (@$old_objects) {
            ...
            for my $col ( @{ $obj->column_names } ) {
                ...
                if ( $col =~ /(\w+)_id$/ ) {
                    ...
                    if ( 'HASH' eq ref( $old->$col ) ) {
                        is( Dumper( $old->$col ), Dumper( $old->$col ),
                            $col );
                    }
                ...
                } ## end else [ if ( $col =~ /(\w+)_id$/)]
            } ## end for my $col ( @{ $obj->column_names...})
            ...
            foreach my $metacol ( keys %$metacolumns ) {
                ...
                } ## end if ( my $type = $metacolumns...)
            } ## end foreach my $metacol ( keys ...)
        } ## end for my $old (@$old_objects)
    } ## end foreach my $name ( keys %$oldies)
} ## end sub checkthemout
