...
$o = MT::Author->load (1);
MT::Author->install_meta ({ column_defs => { my_meta_hash => 'hash meta' }});
...
is($ref_o->my_meta_hash->{something}, 'other', "New meta column has the right HASH value ({'something' => 'other'})");
...
is($ref_o->my_meta_hash->{something}, 'else', "New meta column has the right HASH value ({'something' => 'else'}) after update");
...
$ref_o = MT::Author->load(1);
ok(exists $ref_o->my_meta_hash->{a_new}, "Meta column has a new key in its hash");
