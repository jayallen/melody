...
for my $test (@tests) {
    my ($text, $iso, $utf8) = @{ $test }{qw( text iso utf8 )};
    is(MT::Util::iso_dirify($text,1), $iso, "String '$text' iso_dirifies correctly with hypen as sep")
        if $iso;
    is(MT::Util::utf8_dirify($text,1), $utf8, "String '$text' utf8_dirifies correctly with hypen as sep")
        if $utf8;
}
