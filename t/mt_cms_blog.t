...

{
    ...
    my $static_fileinfos = MT->model('fileinfo')
      ->count( { blog_id => 1, virtual => [ \"= 0", \"is null" ], } );
    is( $static_fileinfos, 36, "all blog #1's fileinfos are static" );

    my $mapped_fileinfos = MT->model('fileinfo')
      ->count( { blog_id => 1, templatemap_id => \"is not null", } );
    ...
    is( MT->model('fileinfo')->count( { blog_id => 1, virtual => 1, } ),
    ...
    is( MT->model('fileinfo')->count( { blog_id => 1, id => 1, } ),
    ...
    is(
        MT->model('fileinfo')
          ->count( { blog_id => 1, id => 1, virtual => 1, } ),
        1,
    ...
    is(
        MT->model('fileinfo')->count(
                 { blog_id => 1, templatemap_id => \"is null", virtual => 1, }
        ),
        0,
