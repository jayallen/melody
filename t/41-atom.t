...
# test blog feed
    {
        ...
        if ( ok( $resp->is_success ) ) {
            ...
            my ($alternate)
              ...
              $feed->links;
            is( $alternate->href, 'http://narnia.na/nana/',
                'blog url is correct' );
            my $entry_count
              = MT::Entry->count( { blog_id => 1 }, { limit => 21 }, );
...
#     $hashed_pwd = shift;
        require Digest::MD5;
        my $hashed_pwd
          = Digest::MD5::md5( join ':', $username, $realm, $chuck_token );
        my $method = 'POST';
        ...
        $entry_id
          || die "Couldn't get entry ID from header Location: " . $location;
