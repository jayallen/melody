...
#    is($asset->class, 'Image', 'class');
    ...
    is( ( $asset->thumbnail_file( { Height => 100, Width => 100 } ) )[0],
        "t/site/assets_c/$cache_path/test-thumb-640x480-1.jpg", 'thumbnail' );
    is( $asset->image_width,  640, 'image_width' );
    is( $asset->image_height, 480, 'height' );
    is( $asset->as_html,
        '<a href="http://narnia.na/nana/images/test.jpg">View image</a>',
        'as_html' );
    is(
        $asset->as_html(
                    { popup => 1, popup_asset_id => $asset->id, include => 1 }
        ),
        qq(<a href="http://narnia.na/nana/images/test.jpg" onclick="window.open('http://narnia.na/nana/images/test.jpg','popup','width=640,height=480,scrollbars=no,resizable=no,toolbar=no,directories=no,location=no,menubar=no,status=no,left=0,top=0'); return false">View image</a>),
        'as_html_popup'
    );
    is(
        $asset->as_html( { include => 1, wrap_text => 1, align => 'right' } ),
    ...
    is( $asset_f->as_html,
        '<a href="http://narnia.na/nana/files/test.tmpl">test.tmpl</a>',
        'as_html' );
    ...
    is( $valid_asset_f->url, 'http://narnia.na/nana/files/test_.tmpl',
        'url' );
    is( $valid_asset_f->description, 'This is a test template.',
        'description' );
