...
sub init_db {
    ...
    require MT::Upgrade;
    my @classes = keys %MT::Upgrade::classes;
    ...
    if ($driver->{dbh}) {
        foreach my $class (@classes) {
            $driver->sql(
                $driver->_drop_table($class),
                $driver->drop_sequence($class),
            );
        }
    } elsif (my $path = $mt->config('DataSource')) {
        # FIXME: Needs to work on Windows too...
        if ((-d $path) && glob("$path/*")) {
