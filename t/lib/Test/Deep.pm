...
sub descend {
    ...
    if ( ref $d1 and ref $d2 ) {
        ...
        if ( $Expects and UNIVERSAL::isa( $d1, "Test::Deep::Cmp" ) ) {
            ...
            return 0
              unless Test::Deep::blessed( Scalar::Util::blessed($d2) )
                  ->descend($d1);
            if ( $d1->can("compare") ) {
...
sub cmp_bag {
    local $Test::Builder::Level = $Test::Builder::Level + 1;
    my $ref = ref( $_[1] ) || "";
    confess "Argument 2 to cmp_bag is not an ARRAY ref ("
      . render_val( $_[1] ) . ")"
      unless $ref eq "ARRAY";
