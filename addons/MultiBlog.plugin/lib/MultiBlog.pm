...
sub preprocess_native_tags {
    ...
    return $result;
} ## end sub preprocess_native_tags
...
sub filter_blogs_from_args {
    ...
    my $arg_count = scalar grep { $_ and $_ ne '' } $args->{include_blogs},
      $args->{blog_id}, $args->{blog_ids}, $args->{exclude_blogs};
    if ( $arg_count > 1 ) {
        $err
          = $plugin->translate(
    ...
    1;
} ## end sub filter_blogs_from_args
...
sub filter_blogs {
    ...
    # System setting does not allow access by default
        ...
        if ( $is_include and $blogs[0] eq "all" ) {
            ...
            my @allow
              = grep { $_ == $this_blog or $access_overrides->{$_} == ALLOWED }
    ...
    } ## end else [ if ($default_access_allowed)]
} ## end sub filter_blogs
