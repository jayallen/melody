...
sub get_auth_info {
    ...
    if ( $app->{is_soap} ) {
        ...
        my $req = $app->get_header('X-WSSE')
          or
          return $app->auth_failure( 401, 'X-WSSE authentication required' );
        $req =~ s/^WSSE //;
        my ($profile);
        ( $profile, $req ) = $req =~ /(\S+),?\s+(.*)/;
...
sub authenticate {
    ...
    $nonce_record->set_values(
                      { id => $auth->{Nonce}, start => time, kind => 'AN' } );
    $nonce_record->save();
...
# xxx Expire sessions on shorter timeout?
    ...
    return $app->auth_failure( 403, 'Invalid login' ) unless $user->is_active;
...
sub new_with_entry {
    ...
    my $mo
      = MT::Atom::Entry::_create_issued( $entry->modified_on, $entry->blog );
...
sub new_post {
    ...
    $entry->status(
        $perms->can_publish_post ? MT::Entry::RELEASE() : MT::Entry::HOLD() );
    ...
    $entry->text(
                  encode_text(
                               MT::I18N::utf8_off( $atom->content()->body() ),
                               'utf-8', $enc
                  )
    );
...
sub _comments_in_atom {
    ...
    } ## end while ( my $comment = $iter...)
    $feed->add_entry($_) foreach @comments;
    $feed;
} ## end sub _comments_in_atom
