...
sub new_with_entry {
    ...
    my $mo = AtomPub::Atom::Entry::_create_issued( $entry->modified_on,
                                                   $entry->blog );
    $atom->set( NS_APP(), 'edited', $mo );

    my $objectasset_iter =
      MT::ObjectAsset->load_iter( {
...
sub new_with_asset {
    ...
    my $mo = AtomPub::Atom::Entry::_create_issued( $asset->modified_on,
                                                   $asset->blog );
    $atom->set( NS_APP(), 'edited', $mo );
...
sub apply_custom_fields {
    ...
    my $field_iter = CustomFields::Field->load_iter(
         { blog_id => [ $obj->blog_id, 0 ], obj_type => $obj->datasource, } );
    while ( my $field = $field_iter->() ) {
        my $value = $atom->get( NS_TYPEPAD(), $field->basename );
        $meta{ $field->basename } = $field->type eq 'checkbox'
          ? $value || q{0}
...
sub associate_assets {
  ...
  LINK: for my $link ( $atom->link ) {
        ...
        require MT::Asset;
        my $a = MT::Asset->load($asset_id) or next LINK;
  ...
  TAG: for my $tag ( $extor->links ) {
        ...
        my $asset = $asset_pkg->load(
                             { blog_id => $blog->id, file_path => $local, } );
        next TAG if !$asset;
...
sub authenticate {
    ...
    if ( my $blog_id = $app->{param}{blog_id} ) {
        ...
        $app->{user} or return $app->error( 403, "Authenticate" );
        ...
        my $perms = $app->{perms} = MT::Permission->load(
             { author_id => $app->{user}->id, blog_id => $app->{blog}->id } );
...
sub publish {
    ...
    unless ($no_ping) {
        $app->ping_and_save( Entry => $entry, Blog => $blog ) or return;
    }
    1;
}
...
sub new_entry {
    ...
    $app->update_entry_from_atom( $entry, $orig_entry, $atom ) or return;
...
sub update_entry_from_atom {
    ...
    if ( my $label = $atom->get( NS_DC, 'subject' ) ) {
        ...
        $cat
          = MT::Category->load(
                               { blog_id => $blog->id, label => $label_enc } )
    ...
    my $body
      = encode_text( MT::I18N::utf8_off( $atom->content->body ), 'utf-8',
                     $enc );
    $entry->text($body);
    ...
    my $message = $orig_entry->id
      ? $app->translate( "User '[_1]' (user #[_2]) added [lc,_4] #[_3]",
                         @message_params )
      : $app->translate( "User '[_1]' (user #[_2]) edited [lc,_4] #[_3]",
                         @message_params );
    require MT::Log;
    $app->log( {
                 message  => $message,
    ...
    return 1;
} ## end sub update_entry_from_atom
...
sub edit_post {
    ...
    $app->update_entry_from_atom( $entry, $entry->clone, $atom ) or return;
...
sub get_posts {
    ...
    $feed->add_link(
          { rel => 'alternate', type => 'text/html', href => $blog->site_url }
    );
    $feed->add_link(
         { rel => 'self', type => $app->atom_x_content_type, href => $uri } );
    $feed->title($blogname);
...
sub delete_post {
    ...
    my %recip =
      $app->publisher->rebuild_deleted_entry( Entry => $entry,
                                              Blog  => $blog );

    # Rebuild archives
    $app->rebuild_archives( Blog => $blog, Recip => \%recip, )
      or die $app->error( $app->errstr );

    # Remove object
    $entry->remove or return $app->error( 500, $entry->errstr );
    '';
} ## end sub delete_post
