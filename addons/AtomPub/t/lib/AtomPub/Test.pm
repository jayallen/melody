...
sub basic_auth {
    return ( Authorization => "Basic "
             . encode_base64( join q{:}, "Chuck D", "seecret" ), );
}
...
sub wsse_auth {
    ...
    my $digest
      = encode_base64( sha1( $nonce_raw . $created . $password ), q{} );
    my $nonce = encode_base64( $nonce_raw, q{} );

    return (
        Authorization => q{WSSE profile="UsernameToken"},
        'X-WSSE' =>
          qq{UsernameToken Username="$username", PasswordDigest="$digest", Nonce="$nonce", Created="$created"},
    );
}
