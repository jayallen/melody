...

{
    ...
    is( $root->nodeName, 'entry',
        "Response creating asset is an Atom entry" );
    ...
    is( $content->getAttribute('type'),
        'image/gif', "Created asset content has the right type" );
    is( $content->getAttribute('src'),
        'http://narnia.na/nana/file', "Created asset content refers to src" );
    is( scalar @{ [ $content->childNodes() ] },
        0, "Created asset content is an empty node" );
}
...
# 'Slug' header used for filename with raw uploads
{
    ...
    is(
        $resp->header('Location'),
        'http://www.example.com/plugins/AtomPub/mt-atom.cgi/1.0/blog_id=1/asset_id=4',
    ...
    is(
        ...
        'http://narnia.na/nana/super-awesome-mega-gif.gif',
...
EOF
    ...
    like( $resp->header('Content-Type'),
          qr{ \A application/atom\+xml }xms,
          "Response creating post is some Atom document" );
    ...
    is(
        $content->textContent(),
        q{<p>This is the post content that goes in the post.</p>},
        "New post had specified HTML content (no additions)"
    );
    ...
    ok( $oa,
        "Creating a new post that related to a created asset associated them"
    );
...
EOF
    ...
    is(
        $location,
        'http://www.example.com/plugins/AtomPub/mt-atom.cgi/1.0/blog_id=1/entry_id=25',
        "Response creating new asset includes API URL of new asset"
    );
    ...
    like(
        $content->textContent(),
        qr{ <p><img \s src="http://narnia.na/nana/super-awesome-mega-gif\.gif"></p> \s+ <p>This\s is\s the\s post\s content\s that\s goes\s in\s the\s post\.</p> }xms,
        "New post had specified HTML content (no additions)"
    );
    ...
    ok( $oa,
        "Creating a new post that merely linked to a created asset associated them"
    );
