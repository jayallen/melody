...
# Set up the test fields.
{
    ...
    ok( MT->model('entry')->has_meta('field.foo'),
        "Field was properly installed as meta source" );
}
...
EOF
    my $resp = run_app(
           ...
           'POST',
           { 'Content-Type' => 'application/atom+xml', basic_auth() },
           $body
    );
    ...
    is( $foo->textContent,
        'homina homina',
        "Response's foo element had expected content" );
}
...
EOF
    ...
    is( $meta->{foo},
        'bar baz woozlebat',
        "Entry's custom field was updated" );
    ...
    is( $foo->textContent,
        'bar baz woozlebat',
        "Edited response's foo element had expected content" );
}
