...
{
    my $resp
      = run_app( 'http://www.example.com/plugins/AtomPub/mt-atom.cgi/1.0',
                 'GET', { basic_auth() } );
    is( $resp->code, 200, "Authorized weblogs request succeeded" );
    like( $resp->header('Content-Type'),
          qr{ \A application/atomsvc\+xml }xms,
          "Authorized weblogs response is a service document" );

    my $doc = XML::LibXML->load_xml( string => $resp->decoded_content );
    my $root = $doc->documentElement;
    is( $root->nodeName, 'service',
        "Service document starts with a service tag" );
    ...
    my ($coll) = @collections;
    is(
        $coll->getAttribute('href'),
        q{http://www.example.com/plugins/AtomPub/mt-atom.cgi/1.0/blog_id=1},
        "Collection has blog 1's AtomPub endpoint"
    );
}

{
    my $resp
      = run_app(
           'http://www.example.com/plugins/AtomPub/mt-atom.cgi/1.0/blog_id=1',
           'GET', { basic_auth() } );
    is( $resp->code, 200, "Posts list request succeeded" );
    like(
          $resp->header('Content-Type'),
          qr{ \A application/atom\+xml }xms,
          "Posts list is an Atom document"
    );
    ...
    my ($entry) = grep {
        $xpath->findvalue( './atom:id', $_ ) eq
          'tag:narnia.na,1978:/nana//1.1'
    } @entries;
    is( $xpath->findvalue( './atom:title', $entry ),
        'A Rainy Day', "Posts list has fixture post #1" );
}
...
EOF
    my $resp = run_app(
        'http://www.example.com/plugins/AtomPub/mt-atom.cgi/1.0/blog_id=1',
        'POST',
        { 'Content-Type' => 'application/atom+xml;type=entry', basic_auth() },
        $body
    );
    is( $resp->code, 201, "New post request succeeded (HTTP Created)" );
    like( $resp->header('Content-Type'),
          qr{ \A application/atom\+xml }xms,
          "Response creating entry is some Atom document" );

    # The existing fixture data means this post will be #24.
    is(
        $resp->header('Location'),
        "http://www.example.com/plugins/AtomPub/mt-atom.cgi/1.0/blog_id=1/entry_id=24",
        "Response creating entry includes API URL of new entry"
    );

    my $doc = XML::LibXML->load_xml( string => $resp->decoded_content );
    my $root = $doc->documentElement;
    is( $root->nodeName, 'entry',
        "Response from new entry is an Atom entry" );
    ...
    my ($content) = @contents;
    is( $content->getAttribute('type'),
        'html', "Response post has HTML content node" );
    is( $content->textContent,
        "<p>my nice post</p>",
        "Response post has correct HTML content" );
    ...
    like( $resp->header('Content-Type'),
          qr{ \A application/atom\+xml }xms,
          "Refetching post returned some Atom document" );

    $doc = XML::LibXML->load_xml( string => $resp->decoded_content );
    $root = $doc->documentElement;
    is( $root->nodeName, 'entry', "Refetching post returned an Atom entry" );

    is( $xpath->findvalue( './atom:id', $root ),
        $id, "Refetched post has same Atom ID as creation response" );
    is( $xpath->findvalue( './atom:updated', $root ),
        $updated,
        "Refetched post has same updated timestamp as creation response" );
    is( $xpath->findvalue( './atom:title', $root ),
        $title, "Refetched post has correct title" );
    ...
    is( $new_content->getAttribute('type'),
        'html', "Refetched post has HTML content" );
    is( $new_content->textContent,
        "<p>my nice post</p>",
        "Refetched post has correct HTML content" );
}
