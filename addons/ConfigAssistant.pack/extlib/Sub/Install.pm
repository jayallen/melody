...
sub _build_public_installer {
    ...
    sub {
        ...
        Carp::croak "named argument 'code' is not optional"
          unless $arg->{code};
        ...
        Carp::croak
          "couldn't determine name under which to install subroutine"
          unless $arg->{as};

        $installer->( @$arg{qw(into as code)} );
      }
} ## end sub _build_public_installer
...
sub exporter {
    ...
    sub {
        ...
        my $target = caller;
        for (@_) {
            Carp::croak "'$_' is not exported by $class"
              if !exists $is_exported{$_};
...
BEGIN {
    *import = exporter( { exports => [qw(install_sub reinstall_sub)] } );
}
