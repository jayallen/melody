...
sub get_options {
    ...
    while ( my ( $key, $val ) = each %o ) {
        $self->{$key} = $$val if ref($val) eq 'SCALAR' || ref($val) eq 'REF';
    }
...
sub setup {
    ...
    eval "use $prereq";
    die(
        "ERROR: Can't handle @{ $self->{'plugin=s@'} } plugin installation: $prereq needed."
    ) if $@ && @{ $self->{'plugin=s@'} };
...
sub make {
    ...
    if ( !$self->{'debug'} && $self->{'export!'} ) {
        chdir( $self->{'export-dir=s'} )
          or die("ERROR: Can't cd to $self->{'export-dir=s'}: $!");
...
sub create_distro_list {
    ...
    for my $lang ( split( /\s*,\s*/, $self->{'lang=s'} ) ) {
        for my $arch ( @{ $self->{'arch=s@'} } ) {
            ...
            my $dist
              = File::Spec->catdir( $self->{'export-dir=s'}, $filename );
            ...
            if ( $self->{'stage'} ) {
                ...
                my $loc
                  = $self->{'prod'}
                  && $self->{'prod-dir=s'} && $dist =~ /$self->{'prod-dir=s'}/
                  ? sprintf( "%s/%s/%s/index.cgi",
                             $self->{'stage-uri=s'},
                             $self->{'prod-dir=s'}, $self->{'export-dir=s'} )
                  : sprintf( "%s/%s/index.cgi",
                            $self->{'stage-uri=s'}, $self->{'export-dir=s'} );
        ...
        } ## end for my $arch ( @{ $self...})
    } ## end for my $lang ( split( /\s*,\s*/...))

    return $distros;
} ## end sub create_distro_list
    ...
    # Otherwise, copy the distribution file(s) to the destination.
        ...
        for my $dist ( @{ $distros->{path} } ) {
              ...
              )
            {
                ...
                $dest = File::Spec->catdir( $self->{'deploy:s'},
                                            scalar fileparse($dist), );
            }
        ...
        } ## end for my $dist ( @{ $distros...})
    } ## end else [ if ( $self->{'deploy:s'...})]
...
sub stage_distro {
    ...
    chdir $stage_root or die("ERROR: Can't chdir to $stage_root $!");
...
CONFIG
    ...
    } ## end unless ( $self->{'ldap'} )

    # Change to the parent of the new stage directory.
    chdir('..') or die("ERROR: Can't chdir to ..") unless $self->{'debug'};
                    ...
                    if ( $type eq 'build' ) {
                        if ( $val eq $id ) {
                            ...
                            $line
                              ...
                              $self->{'stage-uri=s'}, $stage_dir, $suffix;
                        }
                    }
...
sub remove_copy {
    ...
    if ( -d $self->{'export-dir=s'} ) {
        ...
        rmtree( $self->{'export-dir=s'} )
          or die(
            "ERROR: Can't rmtree existing export $self->{'export-dir=s'}: $!")
          unless $self->{'debug'};
    }
}
...
sub set_repo {
    ...
    if (
        $self->{'repo-uri=s'} =~ /http.+?(branches|tags)\/([0-9A-Za-z_.-]+)/ )
    {
    ...
    if ( !$self->{'debug'} && $self->{'export!'} ) {
        ...
        $request->authorization_basic( $self->{'http-user=s'},
                                       $self->{'http-pass=s'} )
          if $self->{'http-user=s'} && $self->{'http-pass=s'};
        my $response = $self->{'agent=s'}->request($request);
        die(
            "ERROR: The repoository '$self->{'repo-uri=s'}' can't be resolved."
...
sub notify {
    ...
    $self->{'email-subject=s'} = sprintf '%s build: %s', $self->{'pack=s'},
      $self->{'stamp=s'};
...
sub inject_footer {
    ...
    my $file
      = $self->{'export!'}
      ? File::Spec->catdir( $self->{'export-dir=s'},
                            $self->{'footer-tmpl=s'} )
      : $self->{'footer-tmpl=s'};
