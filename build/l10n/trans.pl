    ...
    if ( ( $tmpl ne $ARGV ) || eof() ) {
          ...
          )
        {
            ...
            if ( exists $args{phrase} ) {
                ...
                } ## end unless ( $phrase{ $args{phrase...}})
            } ## end if ( exists $args{phrase...})
        } ## end while ( $text =~ ...)
        while ( $text
            =~ /(?:translate|errtrans|trans_error|trans|translate_escape|maketext)\s*\(((?:\s*(?:"(?:[^"\\]+|\\.)*"|'(?:[^'\\]+|\\.)*')\s*\.?\s*){1,})[,\)]/gs
          )
        {
            my ( $msg, %args );
            my $p = $1;
        ...
        } ## end while ( $text =~ /\s*label\s*=>\s*(["'])(.*?)([^\\])\1/gs)
        ...
        } ## end if ( $tmpl =~ /(services|streams)\.yaml$/)
        elsif ( $tmpl =~ /\.yaml$/ ) {
