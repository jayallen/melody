...
if ( !@themes_todo ) {
    ...
    @themes_todo
      = &find_themes_needing_regeneration( $max_tmpl_module_age, @templates );
}
...
# first look at the data files
    ...
    foreach my $templatefile (@templates) {
        ...
        foreach my $theme (@yamls) {
            ...
            if (
                $templateage > $css_age              #template has changed
    ...
    } ## end foreach my $templatefile (@templates)
    @themes_todo = map {$_} sort keys %themes_todo;

    return @themes_todo;
} ## end sub find_themes_needing_regeneration
