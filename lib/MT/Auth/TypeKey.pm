...
sub handle_sign_in {
    ...
    my $entry    = MT::Entry->load($entry_id) or return 0;
...
sub _validate_signature {
    ...
    require MT::Session;
    $dsa_key
      = eval { MT::Session->load( { id => 'KY', kind => 'KY' } ); } || undef;
    if ($dsa_key) {
    ...
    if ( !$dsa_key && $key_location ) {
        ...
        unless ($ua) {
            my $err
              = $app->translate("Couldn't get public key from url provided");
            $app->log( {
                         message  => $err,
                         level    => MT::Log::ERROR(),
                         class    => 'system',
        ...
        $key_cache->set_values(
              { id => 'KY', data => $dsa_key, kind => 'KY', start => time } );
        $key_cache->save();
    } ## end if ( !$dsa_key && $key_location)
    if ( !$dsa_key ) {
        $app->log( {
               message =>
                 $app->translate(
    ...
    my $valid
      = dsa_verify( Key => $dsa_key, Signature => $sig, Message => $msg );
    $timer = time - $timer;
