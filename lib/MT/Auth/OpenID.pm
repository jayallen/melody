...
sub check_openid {
    ...
    unless ($claimed_identity) {
        ...
        return
          $app->errtrans( "Could not verify the OpenID provided: [_1]",
                          $err_msg );
    } ## end unless ($claimed_identity)
    return $claimed_identity;
} ## end sub check_openid
...
sub check_url_params {
    ...
    my $key = $q->param('key') || '';
    $key = MT::Util::encode_url($key) if $key =~ m/[^a-zA-Z0-9_.~%-]/;
    ...
    my $return_to
      ...
      . $static . '&key='
      . $key;
    $return_to .= '&entry_id=' . $entry_id if $entry_id;
    ( trust_root => $path, return_to => $return_to );
} ## end sub check_url_params
