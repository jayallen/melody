...
sub as_html {
    ...
    if ( $param->{include} ) {
        ...
        my $thumb = undef;
        if ( $param->{thumb} ) {
            $thumb = MT::Asset->load( $param->{thumb_asset_id} ) || return
              $asset->error(
                             MT->translate(
                                            "Can't load image #[_1]",
        ...
        if ( $param->{popup} && $param->{popup_asset_id} ) {
            my $popup = MT::Asset->load( $param->{popup_asset_id} ) || return
              $asset->error(
                             MT->translate(
                                            "Can't load image #[_1]",
        ...
        } ## end else [ if ( $param->{popup} &&...)]
    } ## end if ( $param->{include})
    else {
...
sub on_upload {
    ...
    if ( $thumb = $param->{thumb} ) {
        ...
        my $asset_thumb = $img_pkg->load(
                        { file_name => "$base$ext", parent => $asset->id, } );
    ...
    if ( $param->{popup} ) {
        ...
        if (
             my $tmpl =
             MT::Template->load({ blog_id => $blog_id, type => 'popup_image' }
             )
          )
        {
            ...
            my $asset_html = $html_pkg->load(
                         { file_name => "$basename", parent => $asset->id } );
            if ( !$asset_html ) {
