...
while ( my ( $module, $methods ) = each %cms_map ) {
    for my $method (@$methods) {
        my $shim = sub {
            MT::Compat::warn();
            my $h =
              MT->handler_to_coderef( join q{::}, '$Core::MT::CMS', $module,
                                      $method );
            return $h->(@_);
        };
...
sub legacy_init {
    ...
    if (
         my $actions = delete $plugin->{app_action_links}
      ...
      )
    {
        ...
        } ## end foreach my $app ( keys %$actions)
    } ## end if ( my $actions = delete...)
    ...
    if (
         my $actions = delete $plugin->{app_itemset_actions}
    ...
    if (
         my $functions = delete $plugin->{upgrade_functions}
