...
sub is_changed {
    ...
    if ($col) {
        ...
        my $field = $proxy->META_CLASS()->metadata_by_name( $pkg, $col )
          or return 0;
        my $type = $field->{type} or return 0;
...
sub create_meta_object {
    ...
    my $type_id = $field->{type_id} or Carp::croak("no type_id for $col");
    my $id      = $field->{id};
...
sub lazier_load_objects {
    ...
    if ( !exists $proxy->{__objects} && $proxy->{__pkeys} ) {
        my $meta_pkg = $proxy->meta_pkg;
        my @objs
          = $meta_pkg->search( $proxy->{__pkeys}, { fetchonly => ['type'] } );
        for my $obj (@objs) {
