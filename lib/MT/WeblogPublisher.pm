...
sub rebuild_categories {
    ...
    while ( my $cat = $cat_iter->() ) {
        ...
        $mt->_rebuild_entry_archive_type(
            ...
            $param{TemplateMap} ? ( TemplateMap => $param{TemplateMap} ) : (),
            NoStatic => $param{NoStatic},
            Force    => ( $param{Force} ? 1 : 0 ),
        ) or return;
    }
    1;
} ## end sub rebuild_categories
    ...
    for my $at (@at) {
        ...
        if ( $archiver->category_based() ) {
            ...
            for my $cat (@$categories) {
                ...
                } ## end else [ if ( ( $archiver->can(...)))]
            } ## end for my $cat (@$categories)
        } ## end if ( $archiver->category_based...)
        else {
            if (
                 ( $archiver->can('archive_entries_count') )
        ...
        } ## end else [ if ( $archiver->category_based...)]
    } ## end for my $at (@at)

    return %rebuild_recip;
} ## end sub rebuild_deleted_entry
...
sub rebuild_entry {
    ...
    } ## end if ( $param{BuildDependencies...})
...
sub rebuild_archives {
    ...
    for my $at ( keys %$recip ) {
        ...
        if ( $archiver->category_based() ) {
            ...
            } ## end for my $cat_id ( keys %...)
        } ## end if ( $archiver->category_based...)
        elsif ( $archiver->author_based() ) {
            require MT::Author;
            for my $auth_id ( keys %{ $recip->{$at} } ) {
                my $author = MT::Author->load($auth_id) or next;
                if ( $archiver->date_based() ) {
            ...
            } ## end for my $auth_id ( keys ...)
        } ## end elsif ( $archiver->author_based...)
        elsif ( $archiver->date_based() ) {
    ...
    } ## end for my $at ( keys %$recip)

    1;
} ## end sub rebuild_archives
...
sub _rebuild_entry_archive_type {
    ...
    for my $map (@map) {
        ...
        $ctx->{archive_type}         = $at;
        $mt->rebuild_file(
             $blog, $arch_root, $map, $at, $ctx, \my %cond, !$param{NoStatic},
    ...
    1;
} ## end sub _rebuild_entry_archive_type
...
sub rebuild_file {
    ...
    if ( $archiver->entry_based ) {
        $entry = $args{Entry};
        die "$at archive type requires Entry parameter" unless $entry;
        require MT::Entry;
    ...
    if ( $archiver->author_based ) {
        ...
        die "Author-based archive type requires Author parameter"
          unless $args{Author};
        require MT::Author;
        $author = MT::Author->load($author) unless ref $author;
...
# }
    ...
    my $tmpl = MT::Template->load($tmpl_id)
      or return $mt->error(
                 MT->translate( "Could not load template #[_1]", $tmpl_id ) );
    $tmpl->context($ctx);
      ...
      )
    {
        ...
        unless ( $fmgr->exists($path) ) {
            if ( !$fmgr->mkpath($path) ) {
                $timer->unpause if $timer;
                return
                  $mt->trans_error( "Error making path '[_1]': [_2]",
                                    $path, $fmgr->errstr );
            }
        }
        ...
        unless ( defined $fmgr->put_data( $html, $temp_file ) ) {
            $timer->unpause if $timer;
            return
              $mt->trans_error( "Writing to '[_1]' failed: [_2]",
        ...
        if ($use_temp_files) {
            if ( !$fmgr->rename( $temp_file, $file ) ) {
                $timer->unpause if $timer;
                return
                  $mt->trans_error( "Renaming tempfile '[_1]' failed: [_2]",
                                    $temp_file, $fmgr->errstr );
            }
        }
        MT->run_callbacks(
...
sub rebuild_indexes {
    ...
    my $blog;
    $blog = $param{Blog} if defined $param{Blog};
    ...
    while ( my $tmpl = $iter->() ) {
        ...
        unless ($finfo) {
            ...
            my @finfos = MT::FileInfo->load(
                    { blog_id => $tmpl->blog_id, template_id => $tmpl->id } );
            if (    ( scalar @finfos == 1 )
                 && ( $finfos[0]->file_path eq $file )
                 && ( ( $finfos[0]->url || '' ) eq $rel_url ) )
            {
                ...
                foreach (@finfos) { $_->remove(); }
                $finfo =
                  MT::FileInfo->set_info_for_url( $rel_url, $file, 'index',
                          { Blog => $tmpl->blog_id, Template => $tmpl->id, } )
                  || die "Couldn't create FileInfo because "
                  . MT::FileInfo->errstr;
            }
        }
...
sub rebuild_from_fileinfo {
    ...
    my ( $start, $end );
    my $blog = MT::Blog->load( $fi->blog_id ) if $fi->blog_id;
    ...
    my $cat    = MT::Category->load( $fi->category_id ) if $fi->category_id;
    my $author = MT::Author->load( $fi->author_id )     if $fi->author_id;
...
sub publish_future_posts {
    ...
    } ## end foreach my $blog (@blogs)
    $total_changed > 0 ? 1 : 0;
} ## end sub publish_future_posts
...
sub remove_entry_archive_file {
    ...
    unless ($blog) {
        if ($entry) {
            ...
            require MT::Blog;
            $blog = MT::Blog->load( $cat->blog_id ) or return;
        }
    }
    ...
    for my $map (@map) {
        my $file =
          $mt->archive_file_for( $entry, $blog, $at, $cat,
                                 $map,   undef, $auth );
    ...
    1;
} ## end sub remove_entry_archive_file
...
sub queue_build_file_filter {
    ...
    return 0;
} ## end sub queue_build_file_filter
