...
sub mt_new {
    my $cfg
      ...
      : ( $ENV{MT_CONFIG} || $MT::XMLRPCServer::MT_DIR . '/config.cgi' );
...
sub _login {
    ...
    my $perms = MT::Permission->load(
                          { author_id => $author->id, blog_id => $blog_id } );
...
sub _apply_basename {
    ...
    } ## end if ( defined $basename)

    1;
} ## end sub _apply_basename
...
sub _save_placements {
    ...
    if ( $param->{page} ) {
        if ( my $folders = $param->{__permaLink_folders} ) {
            ...
            for my $basename (@$folders) {
                ...
                $parent_id = $folder->id;
            } ## end for my $basename (@$folders)
            @categories = ($folder) if $folder;
        } ## end if ( my $folders = $param...)
    } ## end if ( $param->{page} )
  ...
  CATEGORY: for my $category (@categories) {
    ...
    } ## end for my $category (@categories)

    $changed;
} ## end sub _save_placements
...
sub _new_entry {
    ...
    if ( $mt->{cfg}->NoPublishMeansDraft ) {
        $entry->status( $publish && $perms->can_publish_post
                        ? MT::Entry::RELEASE()
                        : MT::Entry::HOLD() );
    }
    else {
        $entry->status( $perms->can_publish_post
                        ? MT::Entry::RELEASE()
                        : MT::Entry::HOLD() );
    }
...
sub _edit_entry {
    ...
    $entry->status( MT::Entry::RELEASE() )
      if $publish && $perms->can_publish_post;
        ...
        push @res,
          {
            ...
            blogName =>
              SOAP::Data->type(
                string => _encode_text_for_soap( $blog->name, undef, 'utf-8' )
              )
          };
    }
    \@res;
...
sub getUserInfo {
    ...
    die _fault( MT->translate("Invalid login") ) unless $author;
    my ( $fname, $lname )
      = map { encode_text( $_, undef, 'utf-8' ) } split /\s+/, $author->name;
    $lname ||= '';
    {
       userid    => SOAP::Data->type( string => $author->id ),
       firstname => SOAP::Data->type( string => $fname ),
       lastname  => SOAP::Data->type( string => $lname ),
       nickname  => SOAP::Data->type(
                  string =>
                    _encode_text_for_soap( $author->nickname, undef, 'utf-8' )
       ),
...
sub _get_entries {
    ...
    while ( my $entry = $iter->() ) {
        ...
        $row->{ $param{page} ? 'page_id' : 'postid' }
          = SOAP::Data->type( string => $entry->id );
        if ( $class eq 'blogger' ) {
            $row->{content} = SOAP::Data->type( string =>
                      _encode_text_for_soap( $entry->text, undef, 'utf-8' ) );
        }
        else {
            $row->{title} = SOAP::Data->type( string =>
                     _encode_text_for_soap( $entry->title, undef, 'utf-8' ) );
            unless ($titles_only) {
                require MT::Tag;
                my $tag_delim = chr( $author->entry_prefs->{tag_delim} );
                my $tags = MT::Tag->join( $tag_delim, $entry->tags );
                $row->{description} = SOAP::Data->type( string =>
                      _encode_text_for_soap( $entry->text, undef, 'utf-8' ) );
                ...
                $row->{permaLink} = SOAP::Data->type( string => $link ),
                  ...
                  = SOAP::Data->type(
                                      string =>
                                        _encode_text_for_soap(
                                              $entry->basename, undef, 'utf-8'
                                        )
                  );
                $row->{mt_allow_comments}
                  = SOAP::Data->type( int => $entry->allow_comments );
                $row->{mt_allow_pings}
                  = SOAP::Data->type( int => $entry->allow_pings );
                ...
                $row->{mt_keywords} =
                  SOAP::Data->type(
                                    string =>
                                      _encode_text_for_soap(
                                              $entry->keywords, undef, 'utf-8'
                                      )
                  );
                $row->{mt_tags}
                  = SOAP::Data->type(
                      string => _encode_text_for_soap( $tags, undef, 'utf-8' )
                  );
            } ## end unless ($titles_only)
        } ## end else [ if ( $class eq 'blogger')]
        push @res, $row;
    } ## end while ( my $entry = $iter...)
    \@res;
} ## end sub _get_entries
...
sub _delete_entry {
    ...
    # Delete archive file
    my $blog = MT::Blog->load( $entry->blog_id );
    my %recip = $mt->publisher->rebuild_deleted_entry( Entry => $entry,
                                                       Blog => $blog );

    # Rebuild archives
    $mt->rebuild_archives( Blog => $blog, Recip => \%recip, )
...
sub _get_entry {
    ...
    my $basename = SOAP::Data->type(
        string => _encode_text_for_soap( $entry->basename, undef, 'utf-8' ) );
    {
       dateCreated => SOAP::Data->type( dateTime => $co ),
       userid      => SOAP::Data->type( string   => $entry->author_id ),
       ...
       description => SOAP::Data->type(
               string => _encode_text_for_soap( $entry->text, undef, 'utf-8' )
       ),
       title => SOAP::Data->type(
              string => _encode_text_for_soap( $entry->title, undef, 'utf-8' )
       ),
       ...
       mt_convert_breaks =>
         SOAP::Data->type( string => $entry->convert_breaks ),
       mt_text_more => SOAP::Data->type(
                  string =>
                    _encode_text_for_soap( $entry->text_more, undef, 'utf-8' )
       ),
       mt_excerpt => SOAP::Data->type(
            string => _encode_text_for_soap( $entry->excerpt, undef, 'utf-8' )
       ),
       mt_keywords => SOAP::Data->type(
           string => _encode_text_for_soap( $entry->keywords, undef, 'utf-8' )
       ),
       mt_tags => SOAP::Data->type(
                      string => _encode_text_for_soap( $tags, undef, 'utf-8' )
       ),
    };
} ## end sub _get_entry
        ...
        push @data,
          {
            categoryName =>
              SOAP::Data->type(
                string => _encode_text_for_soap( $cat->label, undef, 'utf-8' )
              ),
        ...
        push @data,
          {
            categoryId =>
              SOAP::Data->type(
                   string => _encode_text_for_soap( $cat->id, undef, 'utf-8' )
              ),
            parentId => (
                          $cat->parent_category
                          ? SOAP::Data->type(
                                 string =>
                                   _encode_text_for_soap(
                                     $cat->parent_category->id, undef, 'utf-8'
                                   )
                            )
                          : undef
            ),
            ...
            description =>
              SOAP::Data->type(
                  string =>
                    _encode_text_for_soap( $cat->description, undef, 'utf-8' )
              ),
            htmlUrl => SOAP::Data->type(
                       string => _encode_text_for_soap( $url, undef, 'utf-8' )
            ),
          };
    } ## end while ( my $cat = $iter->...)
    \@data;
} ## end sub getCategories
...
sub getTagList {
    ...
    while ( my $tag = $iter->() ) {
        push @data,
          {
            tagName => SOAP::Data->type(
                 string => _encode_text_for_soap( $tag->name, undef, 'utf-8' )
            ),
        ...
        push @data,
          {
            pingTitle =>
              SOAP::Data->type(
                       string =>
                         _encode_text_for_soap( $ping->title, undef, 'utf-8' )
              ),
...
} ## end sub publishScheduledFuturePosts
...
sub newMediaObject {
    ...
    if ( $ext =~ m/(jpe?g|png|gif|bmp|tiff?|ico)/i ) {
        ...
        if ( MT::Image->has_html_signature( path => $local_file ) ) {
            die _fault(
                        MT->translate(
                                       "Saving [_1] failed: [_2]",
                                       $local_basename,
