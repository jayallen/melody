...
sub search_terms {
    ...
    my %terms = exists( $params->{terms} ) ? %{ $params->{terms} } : ();
    ...
    if (@or_tags) {
        $join_on_arg
          = &add_join( $ot_class, { object_datasource => $class->datasource },
                       {}, $depth, \@or_tags );
    }
