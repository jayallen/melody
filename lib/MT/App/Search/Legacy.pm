...
sub throttle_response {
    ...
    my $msg
      = $app->translate( "You are currently performing a search. Please wait "
...
sub throttle_control {
    ...
    if ( eval { require DB_File; 1 } ) {
        my $file
          = File::Spec->catfile( $app->config('TempDir'), 'mt-throttle.db' );
...
sub execute {
    ...
    return $app->error( $app->translate('Invalid request.') ) unless $blog;
    ...
    if ( defined( $ctx->stash('content_type') ) ) {
        ...
        } ## end if ( $app->{searchparam...})
        else {
            $app->send_http_header( $ctx->stash('content_type') );
            $app->print($res);
        }
    } ## end if ( defined( $ctx->stash...))
    $res;
} ## end sub execute
...
sub _tag_search {
    ...
    while ( my $entry = $iter->() ) {
        ...
        if ( $hits{$blog_id} && $hits{$blog_id} >= $max ) {
            my $blog  = $blogs{$blog_id} || MT::Blog->load($blog_id) or next;
...
sub query_parse {
    ...
    while ( my $atom = shift @tokens ) {
        ...
        push @search_keys, { atom => quotemeta($atom), type => $type };
    } ## end while ( my $atom = shift ...)
