...
sub generate_captcha {
    ...
    $sess->start(time);
    $sess->name($token);
    $sess->save or $app->error( $sess->errstr ), return undef;
...
sub _makerandom {
    ...
    if ( -e "/dev/urandom" ) {
        my $fh;
        open( $fh, '/dev/urandom' ) or die "Couldn't open /dev/urandom";
...
sub _generate_captcha {
    ...
    my $im
      = $imbase->Montage( geometry => $geometry_str, tile => $tile_geom );
    $im->Blur();
