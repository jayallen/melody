...
__PACKAGE__->install_properties( {
       column_defs => {
           ...
           'text' =>
             { type => 'text', label => 'Template Text', revisioned => 1 },
           ...
           'build_dynamic' =>
             { type => 'boolean', label => 'Dynamicity', revisioned => 1 },
           'identifier' => 'string(50)',
           'build_type' =>
             { type => 'smallint', label => 'Build Type', revisioned => 1 },
           'build_interval' =>
             { type => 'integer', label => 'Interval', revisioned => 1, },
...
sub build {
    ...
    my $tokens = $tmpl->tokens or return;
    ...
    } ## end if ( my $blog_id = $tmpl...)
    $page_layout = $tmpl->page_layout if $tmpl->page_layout;
    $ctx->var( 'page_layout', $page_layout ) unless $ctx->var('page_layout');
...
sub save_widgetset {
    ...
    $obj->modulesets( join ',', @ids ) if scalar @ids != scalar @inst;
    $obj->text($text);
    return $obj->SUPER::save;
} ## end sub save_widgetset
...
sub set_values_internal {
    ...
    } ## end if ( exists $cols->{text...})
    $tmpl->SUPER::set_values_internal(@_);
} ## end sub set_values_internal
...
sub _sync_from_disk {
    ...
    unless ( File::Spec->file_name_is_absolute($lfile) ) {
        if ( $tmpl->blog_id ) {
            my $blog = MT::Blog->load( $tmpl->blog_id ) or return;
...
sub post_remove_widget {
    ...
    my $iter = MT::Template->load_iter(
                 { blog_id => [ $tmpl->blog_id, 0 ], type => 'widgetset', } );
    my @resave;
    while ( my $ws = $iter->() ) {
