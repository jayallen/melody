...
sub save {
    ...
    my $blog    = MT->model('blog')->load( $map->blog_id ) or return;
    my $blog_at = $blog->archive_type;
    my @ats = map {$_} grep { $map->archive_type ne $_ } split /,/, $blog_at
      if $blog_at ne 'None';
...
sub remove {
    ...
    if ( ref $map ) {
        ...
        if ($remaining) {
            ...
            if ( $at && $at ne 'None' ) {
                my @newat
                  = map {$_} grep { $map->archive_type ne $_ } split /,/, $at;
        ...
        if ( $_[0] && $_[0]->{template_id} ) {
            ...
            if ($tmpl) {
                return $result
                  unless $tmpl->blog_id;  # global template does not have maps
        ...
        while ( my $blog = $iter->() ) {
            ...
            for my $at ( @{ $ats{ $blog->id } } ) {
                  ...
                  )
                {
                    my $remaining =
                      __PACKAGE__->load(
                               { blog_id => $blog->id, archive_type => $at, },
                               { limit   => 1, } );
                    if ($remaining) {
                        $remaining->is_preferred(1);
                        $remaining->save;
...
sub _prefer_next_map {
    ...
    my @all = MT::TemplateMap->load(
           { blog_id => $map->blog_id, archive_type => $map->archive_type } );
