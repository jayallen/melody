...
sub send {
    ...
    unless ($@) {
        foreach my $header ( keys %hdrs ) {
            ...
            if ( ref $val eq 'ARRAY' ) {
                foreach (@$val) {
                    if (    ( lc($mail_enc) ne 'iso-8859-1' )
                         || (m/[^[:print:]]/) )
                    {
                        if ( $header =~ m/^(From|To|Reply|B?cc)/i ) {
                            if (m/^(.+?)\s*(<[^@>]+@[^>]+>)\s*$/) {
                                $_ =
                                  MIME::EncWords::encode_mimeword(
                                        encode_text( $1, $enc, lc $mail_enc ),
                                        'b', lc $mail_enc )
                                  . ' '
                                  . $2;
                            }
                          ...
                          )
                        {
                            $_ =
                              MIME::EncWords::encode_mimeword(
                                        encode_text( $_, $enc, lc $mail_enc ),
                                        'b', lc $mail_enc );
                        }
                    } ## end if ( ( lc($mail_enc) ne...))
                } ## end foreach (@$val)
            } ## end if ( ref $val eq 'ARRAY')
            else {
                if (    ( lc($mail_enc) ne 'iso-8859-1' )
                     || ( $val =~ /[^[:print:]]/ ) )
                {
                      ...
                      )
                    {
                        $hdrs{$header}
                          = MIME::EncWords::encode_mimeword(
                                      encode_text( $val, $enc, lc $mail_enc ),
                                      'b', lc $mail_enc );
                    }
                } ## end if ( ( lc($mail_enc) ne...))
            } ## end else [ if ( ref $val eq 'ARRAY')]
        } ## end foreach my $header ( keys %hdrs)
    } ## end unless ($@)
    else {
