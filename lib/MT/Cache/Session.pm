...
sub delete {
    my MT::Cache::Session $self = shift;
    my ( $key, $time ) = @_;
    MT::Session->remove( { id => $key, kind => $self->{kind} } ) or return 0;
    1;
}
*remove = \&delete;
...
sub _set {
    ...
    my $cache = MT::Session->load( { id => $key, kind => $self->{kind}, } );
    $cache->remove() if $cache;
    $cache = MT::Session->new;
    $cache->set_values(
        { id => $key, kind => $self->{kind}, start => time, data => $val, } );
    $cache->save();
}
