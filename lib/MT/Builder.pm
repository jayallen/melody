    ...
    for my $t (@$tokens) {
        if ( $t->[0] eq 'TEXT' ) {
            ...
            if ($h) {
                ...
                $out = $ph->( $ctx, \%args, $out, \@args ) if %args && $ph;
                $res .= $out if defined $out;

                if ($timer) {
                    $timer->mark(
                          "tag_" . lc( $t->[0] ) . args_to_string( \%args ) );
                }
            } ## end if ($h)
            else {
                if ( $t->[0] !~ m/^_/ ) {    # placeholder tag. just ignore
                    return $build->error(
                        MT->translate( "Unknown tag found: [_1]", $t->[0] ) );
                }
            }
        } ## end else [ if ( $t->[0] eq 'TEXT')]
    } ## end for my $t (@$tokens)
...
sub args_to_string {
    ...
    } ## end if ( $more_args && @$more_args)
    return $str ne '' ? '[' . substr( $str, 1 ) . ']' : '';
} ## end sub args_to_string
1;
__END__
