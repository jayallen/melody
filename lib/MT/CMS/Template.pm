...
sub itemset_stc {
    return itemset_handler( @_, MT::PublishOption::ONDEMAND() );
}

sub itemset_dyn {
    return itemset_handler( @_, MT::PublishOption::DYNAMIC() );
}

sub itemset_dis {
    return itemset_handler( @_, MT::PublishOption::DISABLED() );
}

sub itemset_man {
    return itemset_handler( @_, MT::PublishOption::MANUALLY() );
}
...
sub edit {
            ...
            foreach my $tag (@$includes) {
                ...
                if ($other) {
                    ...
                    if ( $other->{errors} && @{ $other->{errors} } ) {
                        $param->{error}
                          = $app->translate(
                            "One or more errors were found in included template module ("
                              . $other->name
                              . ")." );
                        $param->{error} .= "<ul>\n";
                    ...
                    push @widget_sets,
                      {
                        include_link =>
                          $app->mt_uri(
                            mode => 'edit_widget',
                            args =>
                              { blog_id => $wset->blog_id, id => $wset->id, },
                          ),
                        include_module => $name,
                      };
                }
                    ...
                    push @widget_sets,
                      {
                        create_link =>
                          $app->mt_uri(
                               mode => 'edit_widget',
                               args => { blog_id => $blog_id, name => $name },
                          ),
                        include_module => $name,
                      };
                }
    ...
    } ## end foreach my $tag_set (@$all_tags)
    ...
    $param->{dirty} = 1 if $q->param('dirty');
...
sub create_preview_content {
    ...
    return @obj;
} ## end sub create_preview_content
...
sub _populate_archive_loop {
    ...
    foreach my $map_obj (@tmpl_maps) {
        ...
        $map->{ 'map_schedule_period_' . $period } = 1 if defined $period;
        $map->{map_schedule_interval} = $interval if defined $interval;
        ...
        foreach (@$tmpl_loop) {
                 ...
                 || ( $map->{file_template} eq $_->{value} ) )
            {
                $_->{selected}        = 1;
                $custom               = 0;
                $map->{file_template} = $_->{value} if !$map->{file_template};
            }
        }
        if ($custom) {
            unshift @$tmpl_loop,
        ...
        push @maps, $map;
    } ## end foreach my $map_obj (@tmpl_maps)
    @maps = sort { MT::App::CMS::archive_type_sorter( $a, $b ) } @maps;
    return \@maps;
} ## end sub _populate_archive_loop
...
sub pre_save {
    ...
    my $cache_expire_type
      = defined $q->param('cache_expire_type')
         ...
         || $type eq 'widgetset' )
    {
        ...
        for my $p (@p) {
            ...
            if ( $p =~ /^archive_tmpl_preferred_(\w+)_(\d+)$/ ) {
                my $at     = $1;
                my $map_id = $2;
                $map = MT::TemplateMap->load($map_id) or next;
...
} ## end sub dialog_publishing_profile
...
} ## end sub dialog_refresh_templates
...
sub refresh_all_templates {
  ...
  BLOG: for my $blog_id (@id) {
        ...
        } ## end if ( $refresh_type eq ...)

        # Load default templates for the given template set, if any.
        if ($blog_id) {
            $tmpl_list = MT::DefaultTemplates->templates( $template_set
                                                     || $blog->template_set );
        }
...
sub refresh_individual_templates {
    ...
    foreach my $tmpl (@$tmpl_list) {
        ...
        $tmpl_ids->{ $tmpl->{identifier} } = $tmpl if $tmpl->{identifier};
        if ( $tmpl->{type}
             !~ m/^(archive|individual|page|category|index|custom|widget)$/ )
        {
...
sub clone_templates {
    ...
    foreach my $tmpl_id (@id) {
        ...
        my $new_tmpl = $tmpl->clone(
                    { Except => { id => 1, name => 1, identifier => 1, }, } );
        ...
        $new_tmpl->save
          or return
          $app->errtrans( 'Could not clone template: [_1]',
                          $new_tmpl->errmsg );
...
} ## end sub publish_templates_from_search
...
} ## end sub publish_index_templates
...
sub publish_archive_templates {
    ...
    return $app->error( $app->translate("Invalid request.") ) unless @ids;
    my $tmpl_id;
    my %ats;
    require MT::TemplateMap;
    while ( !$tmpl_id && @ids ) {
...
} ## end sub publish_archive_templates
...
sub list_widget {
    ...
    while ( my $widgetset = $iter->() ) {
        ...
        my $ws = { id => $widgetset->id, widgetmanager => $widgetset->name, };
...
sub restore_widgetmanagers {
    ...
    1;
} ## end sub restore_widgetmanagers

{
    my @period_options = (
                           { name => 'minutes', expr => 60, },
                           { name => 'hours',   expr => 60 * 60, },
                           { name => 'days',    expr => 24 * 60 * 60, },
    );
