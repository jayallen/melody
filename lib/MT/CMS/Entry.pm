...
sub list {
    ...
    } ## end unless ( $app->user->is_superuser)
    ...
    if ( $filter_col && $filter_val ) {
        ...
        if ( !exists( $terms{$filter_col} ) ) {
            if ( $filter_col eq 'category_id' ) {
                ...
                $arg{'join'} = MT->model('objectasset')->join_on( 'object_id',
                              { object_ds => $type, asset_id => $filter_val },
                              { unique    => 1 } );
            }
                    ...
                    || ( $filter_col eq 'exacttag' ) )
            {
                ...
                if ( $pkg->is_meta_column($filter_col) ) {
                    ...
                    my $meta_terms
                      = { $type_col => $filter_val, type => $type_id, };
            ...
            } ## end else [ if ( $filter_col eq 'category_id')]
            $param{filter_args}
              = "&filter="
    ...
    while ( my $au = $iter->() ) {
        next if $seen{ $au->id };
        $seen{ $au->id } = 1;
        my $row = { author_name => $au->name, author_id => $au->id };
        push @authors, $row;
        if ( @authors == 50 ) {
            $iter->end;
            last;
    ...
    $param{can_republish}
      = $blog_id ? $perms->can_rebuild : $app->user->is_superuser ? 1 : 0;
