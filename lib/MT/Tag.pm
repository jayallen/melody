...
__PACKAGE__->install_properties( {
       ...
       defaults      => { n8d_id => 0, is_private => 0, },
    ...
    return {
        ...
        _blog => {
            ...
            terms => sub {
                ...
                my $join_str = '= tag_id';
                push @{ $db_args->{joins} },
                  MT->model('objecttag')->join_on( undef,
                               { blog_id => $blog_id, tag_id => \$join_str, },
                               { unique  => 1, },
                  );
                return;
            },
                ...
                MT->model('objecttag')->count( {
                     ...
                     },
                     {
                       join =>
                         MT::Entry->join_on(
                            undef,
                            { class => $prop->entry_class, id => \$join_str, }
                         ),
                     }
                );
            },
...
#            view      => [ 'website', 'blog' ],
            ...
            terms     => sub {
                ...
                $db_args->{joins} ||= [];
                push @{ $db_args->{joins} },
                  MT->model('objecttag')->join_on( 'tag_id',
                                      { object_datasource => 'asset', },
                                      { group => ['tag_id'], unique => 1, } );
                return;
            },
        },
...
sub normalize {
    ...
    my $private = $str =~ m/^@/;
    $str = MT::I18N::encode_text( $str, MT->instance->config->PublishCharset,
                                  'utf-8' );
    $str =~ s/[@!`\\<>\*&#\/~\?'"\.\,=\(\)\${}\[\];:\ \+\-\r\n]+//gs;
    $str = lc $str;
    $str = '@' . $str if $private;
    $str = MT::I18N::encode_text( $str, 'utf-8',
                                  MT->instance->config->PublishCharset );
    $str;
} ## end sub normalize
...
sub remove {
    ...
    $tag->SUPER::remove(@_) or return $tag->error( $tag->errstr );
    ...
    if ($n8d_tag) {
        ...
        if ( !MT::Tag->exist( { n8d_id => $n8d_tag->id } ) ) {
            ...
            if ( !MT::ObjectTag->exist( { tag_id => $n8d_tag->id } ) ) {
                $n8d_tag->remove or return $tag->error( $n8d_tag->errstr );
            }
        }
    }
    1;
...
sub clear_cache {
    ...
    my $sess_id
      ...
      . ( $param{private} ? ';private' : '' );
    require MT::Session;
    $tag_cache = MT::Session->load( { kind => 'TC', id => $sess_id } );
...
sub has_tag {
    ...
    foreach (@tags) {
        ...
        return 1
          if ( $tag ne $n8d_tag ) && ( $n8d_tag eq MT::Tag->normalize($_) );
    }
    0;
}
                    ...
                    undef,
                    {
                       join =>
                         MT::ObjectTag->join_on(
                                 ...
                                 {
                                   unique => 1,
                                   join => $pkg->join_on( undef, $pkg_terms )
                                 }
                         )
                    }
    );
