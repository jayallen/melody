    ...
    for my $class_hash (@$obj_to_backup) {
        ...
        while (1) {
            ...
            while ( my $object = $iter->() ) {
                ...
                my $bytes_to_add = length $printer->(
...
sub parents {
    my $obj = shift;
    { blog_id => MT->model('blog'), parent => MT->model('asset') };
}
...
sub parents {
    my $obj = shift;
    { blog_id => MT->model('blog'), tb_id => MT->model('trackback'), };
}
...
sub parents {
    my $obj = shift;
    { blog_id => MT->model('blog'), template_id => MT->model('template') };
}
