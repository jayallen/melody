    ...
    if ( ref $val eq 'HASH' ) {
        ...
        if ( $stmt->like->{$col} ) {
            if ( ref($val) eq 'HASH' ) {
                ...
                $val = { op => 'LIKE', value => $val, };
            }
        }
    } ## end else [ if ( ref $val eq 'HASH')]
