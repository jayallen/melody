...
TBLSQL
            ...
            $sth_tbl->finish;
            my $sql_tbl = $rows_tbl->{$table_name}->{'sql'} or next;
...
sub column_defs {
    ...
    while ( my $row = $sth->fetchrow_hashref ) {
        ...
        $defs->{$colname}{not_null} = 1 if $row->{notnull};
    ...
    if ($indexes) {
        foreach my $name ( keys %$indexes ) {
            ...
            if ( ref $indexes->{$name} eq 'HASH' ) {
                ...
                next
                  unless exists( $idx_info->{unique} ) && $idx_info->{unique};
