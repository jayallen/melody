...
sub insert_from_sql {
    ...
    foreach my $col (@fields_from) {
        ...
        if ( defined $def ) {
            if ( $ddl->type2db($def) ne $ddl->type2db( $db_defs->{$col} ) ) {
                $from_fields
                  .= $ddl->cast_column_sql( $class, $col, $db_defs->{$col} );
            }
            else {
    ...
    if (@new_fields) {
        ...
        foreach my $col (@new_fields) {
            ...
            $new_fields .= $value;
        } ## end foreach my $col (@new_fields)
        $sql .= $new_fields;
    } ## end if (@new_fields)
    $sql .= "\nFROM ${table_name}_upgrade";

    return $sql;
} ## end sub insert_from_sql
...
sub index_column_sql {
    ...
    } ## end elsif ( ref $indexes->{$name...})
    return @stmts;
} ## end sub index_column_sql
