...
sub summarize {
    ...
    my $super_registry
      = ( $type_id ne $obj->datasource )
      ? MT->registry( summaries => $obj->datasource )
      : {};
    my $regen 
      = $params{code}
...
sub expire_summary {
    ...
    if ( defined($terms) ) {
        ...
        my $params     = { $type_key => $obj->id, type => $terms->{type}, };
        my $new        = 1;
        my ($summary)
