...
sub PORTAL_URL() {
    '';
}    # default PORTAL_URL is determined in building packages
...
sub guess_encoding_encode {
    ...
    if ( ref($dec) ) {
        ...
        for
          my $encode_name ( MT->config('PublishCharset'), @ENCODINGS_ENCODE )
        {
...
sub encode_text_encode {
    ...
    if ( $from ne $to ) {
        ...
        eval {
            if ( (
                   ( 'iso-2022-jp' eq lc($to) ) || ( 'shift_jis' eq lc($to) )
                 )
                 && ( 'utf-8' eq lc($from) )
              )
            {
...
=head2  encode_text_encode

=head2  encode_text_perl
...
=head2  guess_encoding_encode

=head2  guess_encoding_perl

=head2  length_text

=head2  length_text_encode

=head2  length_text_perl

=head2  lowercase

=head2  lowercase_encode

=head2  lowercase_perl

=head2  substr_text

=head2  substr_text_encode

=head2  substr_text_perl

=head2  uppercase

=head2  uppercase_encode

=head2  uppercase_perl
