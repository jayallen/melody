...
} ## end sub substr_text_jcode

sub lowercase_jcode {
    my $class = shift;
    my ( $str, $enc ) = @_;
    $enc = $class->_set_encode( $str, $enc );
    $str = $class->encode_text_jcode( $str, $enc, 'utf-8' )
      if lc $enc ne 'utf-8';
    $class->utf8_on_jcode($str);
    $str = lc $str;
    $class->utf8_off_jcode($str);
    $str = $class->encode_text_jcode( $str, 'utf-8', $enc )
      if lc $enc ne 'utf-8';
    return $str;
}
...
sub wrap_text_encode {
    ...
    $text
      = $class->wrap_text_jcode( $text, $cols, $tab_init, $tab_sub, $enc );
    $text;
} ## end sub wrap_text_encode
