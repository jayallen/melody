...
%EXPORT_TAGS
  = (
     constants => [qw(POLICY_MOD_NOONE POLICY_MOD_ANYONE POLICY_MOD_AUTHONLY)]
  );
...

{
    ...
    sub set_defaults {
        ...
        } ## end unless ($default_text_format)
...
sub create_default_templates {
    ...
    for my $val (@$tmpl_list) {
        ...
        if ( ( 'widgetset' eq $val->{type} ) && ( exists $val->{widgets} ) ) {
            ...
            $obj->modulesets( MT->model('template')
                          ->widgets_to_modulesets( $modulesets, $blog->id ) );
        }
    ...
    $app->set_language($curr_lang);
    return $blog;
} ## end sub create_default_templates
...
sub current_timestamp {
    ...
    return sprintf '%04d%02d%02d%02d%02d%02d', $ts[5] + 1900, $ts[4] + 1,
      @ts[ 3, 2, 1, 0 ];
}
...
sub has_archive_type {
    ...
    my @maps = MT::TemplateMap->load(
                            { blog_id => $blog->id, archive_type => $type } );
...
sub count_static_templates {
    ...
    require MT::TemplateMap;
    my @maps = MT::TemplateMap->load(
                    { blog_id => $blog->id, archive_type => $archive_type } );
    return 0 unless @maps;
    require MT::PublishOption;
    foreach my $map (@maps) {
...
sub clone_with_children {
    ...
    if ( ( !exists $classes->{'MT::Entry'} ) || $classes->{'MT::Entry'} ) {
        ...
        require MT::Entry;
        $iter
          = MT::Entry->load_iter( { blog_id => $old_blog_id, class => '*' } );
        while ( my $entry = $iter->() ) {
            $callback->(
                         $state . " "
                           . MT->translate(
         ...
         || $classes->{'MT::Template'} )
    {
        ...
        while ( my $tmpl = $iter->() ) {
            ...
            $tmpl_processor->( $new_blog_id, \$counter, $tmpl,
                               $new_tmpl,    \%tmpl_map );
        }
        $iter = MT::Template->load_iter(
...
sub use_revision {
    ...
    return $blog->SUPER::use_revision(@_) if 0 < scalar(@_);
